...
describe('PATCH /forms/<formID or formName>/submissions/<submissionId>', () => {
  before(async () => {
  ...
  it('should return submission data', () => supertest(sails.hooks.http.app)
      .patch(`/forms/${formName}/submissions/${submission}`)
      .set({ authorization_token: token })
      .send({ firstName: 'Banchan123' })
      .expect(200)
      .expect((res) => {
        expect(res.body.data.data).to.have.property('firstName', 'Banchan123');
      }));
  it('should throw formioException with message Form does not exist', () => supertest(sails.hooks.http.app)
      .patch(`/forms/nonExistant-form/submissions/${submission}`)
      .set({ authorization_token: token })
      .send()
      .expect(502)
      .expect((res) => {
  ...
  it('should throw formioException with message Not Found when there is no submission corresponding to the provided ID', () => supertest(sails.hooks.http.app)
      .patch(`/forms/${formId}/submissions/5d380af21efea731ddb1ea24`)
      .set({ authorization_token: token })
      .send()
      .expect(502)
      .expect((res) => {
